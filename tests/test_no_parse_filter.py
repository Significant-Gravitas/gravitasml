...
class TestNoParseFilter(unittest.TestCase):
    ...
    def test_no_parse_with_multiple_nested(self):
        ...
        markup = (
            "<tag | no_parse><tag><tag>deep</tag></tag><content>more</content></tag>"
        )
        result = parse_markup(markup)
    ...
    def test_no_parse_multiple_root_tags(self):
        ...
        result = parse_markup(markup)
        expected = [
            {"tag": "<inner>content1</inner>"},
            {"tag": "<inner>content2</inner>"},
        ]
        self.assertEqual(expected, result)
    ...
    def test_no_parse_mixed_with_normal_tags(self):
        ...
        result = parse_markup(markup)
        expected = {
            "root": {"normal": "parsed", "no_parse": "<inner>not parsed</inner>"}
        }
        self.assertEqual(expected, result)
    ...
    def test_no_parse_with_complex_html_like_content(self):
        ...
        result = parse_markup(markup)
        expected = {
            "template": '<div class="container"><p>Hello <strong>world</strong>!</p><ul><li>Item 1</li><li>Item 2</li></ul></div>'
        }
        self.assertEqual(expected, result)
    ...
    def test_no_parse_deeply_nested(self):
        ...
        object = parser.parse()
        correct = {
            "outer": "<level1><level2><level3>deep content</level3></level2></level1>"
        }
        self.assertEqual(correct, object)
    ...
    def test_no_parse_multiple_nested_same_name(self):
        ...
        object = parser.parse()
        correct = {
            "container": "<container>inner1</container><container>inner2</container>"
        }
        self.assertEqual(correct, object)
